...
re-verified on boot — a dead one re-queues its row for re-publish, while
a live media one has its stored URL refreshed from its CDN message
...
class Assets:
    ...
    async def sync_cdn(self, _event: hikari.StartedEvent) -> None:
        ...
        1. **Verify** — every published media row's CDN message (and every
           ...
           below, while a live one **refreshes** the stored URL from the
    ...
    async def _verify_published(
        ...
        for row in rows_to(AssetRow, rows):
            ...
            # current attachment URL replaces the row's stale one — the bot
            # pulls the pre-existing CDN reference instead of re-uploading
            # the media on each boot. A message that lost its attachment
            # (edited) is dead too and re-queues the publish.
            if not message.attachments:
                _log.info(
                    "asset %s CDN message lost its attachment; "
                    + "re-queuing publish",
                    row.key,
                )
                await self.bot.db.execute(
            ...
            if fresh_url != row.url:
                _log.info(
                    "asset %s CDN URL refreshed (%s)", row.key, fresh_url
                )
                await self.bot.db.execute(
