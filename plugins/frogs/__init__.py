...
class FrogsPlugin(Plugin):
    ...
    # no depends_on: frogs touches no experience table (2026-09 — exp is
    # chatting-only, consumes grant statuses). The old frogs -> experience
    # edge was for the FROG exp writes and is gone with them, so disabling
    # the chat ladder no longer takes the frog side down with it.
