...
class DevCog(commands.Cog):
    ...
    async def owner(self, ctx: commands.Context[CazzuBot]) -> None:
        _log.info("%s is the bot owner.", ctx.author)
        await ctx.send(f"You are {ctx.author.mention}!")

    @commands.hybrid_group()
    async def calc(self, _ctx: commands.Context[CazzuBot]) -> None:
        """Helpers for level math."""

    @calc.command(name="to")
    async def calc_to(
        self, ctx: commands.Context[CazzuBot], n: int
    ) -> None:
    ...
    @commands.hybrid_command()
    async def scrape(self, ctx: commands.Context[CazzuBot]) -> None:
...
class HotswapCog(commands.Cog):
    ...
    @commands.hybrid_group()
    async def cog(self, _ctx: commands.Context[CazzuBot]) -> None:
        """Plugin hotswap."""

    @cog.command(name="reload")
    async def plugin_reload(
        self, ctx: commands.Context[CazzuBot], *, plugin_name: str
    ) -> None:
        if plugin_name not in [p.name for p in self.bot.plugins]:
    ...
    async def plugin_load(
        ...
        if plugin_name in [p.name for p in self.bot.plugins]:
