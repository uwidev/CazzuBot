	...
	*,
	...
	headers_s = f"{' ' * spacing}".join(
		header_format.format(
			val=headers[i], align=align[i], pad=padding[i]
		)
	...
	for row in range(len(entries)):
		...
		rows_s.append(
			f"{(' ' if row % 2 else fill) * spacing}".join(row_raw)
		)
...
def highlight_row(
	...
	*,
	...
	scoreboard[index + 1] = (
		"@"
		+ this_rank
		+ scoreboard[index + int(has_header)][col1_width + 1 :]
	)
	return scoreboard
...
def calc_max_col_width(
	...
	for col in range(len(entries[0])):
		if isinstance(entries[0][col], str):
			entire_col = list(
				entries[row][col] for row in range(len(entries))
			)
		else:  # is a number, take into account comma
			entire_col = list(
				f"{entries[row][col]:,}" for row in range(len(entries))
			)
