...
class SnowflakeTable(ABC):
	...
	def values(self):
		...
		joined = ", ".join(map(str, self.__dict__.values())).replace(
			"None", "NULL"
		)
		return f"({joined})"
...
class FrogSpawn(SnowflakeTable):
	...
	def __iter__(self):
		return iter(
			[self.gid, self.cid, self.interval, self.persist, self.fuzzy]
		)
...
class MemberFrog(SnowflakeTable):
	...
	def __iter__(self):
		"""Unpacking for inserting new row."""
		return iter(
			[self.gid, self.gid, self.gid, self.normal, self.frozen]
		)
