	...
	async def confirm(ctx: commands.Context) -> bool:
		...
		try:
			reaction, _ = await ctx.bot.wait_for(
				"reaction_add", check=check, timeout=7
			)
...
async def find_username(
	bot: CazzuBot, ctx: commands.Context, uid: int
) -> str:
