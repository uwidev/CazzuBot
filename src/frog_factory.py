...
async def spawn_and_wait(
	...
	*,
	...
	try:
		...
		frog_type = (
			db.table.FrogTypeEnum.NORMAL
		)  # for now until fancy frogs
...
async def queue_frog_spawns(
	bot: CazzuBot, frog_spawns: list[db.table.FrogSpawn]
):
	...
	now = pendulum.now()
	run_ats = [
		roll_future_frog(now, frog.interval, frog.fuzzy)
