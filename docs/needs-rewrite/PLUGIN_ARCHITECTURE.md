...
`bot.events.on()` returns an **unsubscribe token**; `off(event_type, handler)`
removes a registration. Bus subscriptions are deferred effects — a plugin hands
the token to the lifecycle so unload withdraws its interest. This is the
reactive-coeffect idea in miniature: an observer declares interest in a context
change and is detached when deactivated.
