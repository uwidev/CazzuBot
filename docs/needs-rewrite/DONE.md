...
I think currently the code only looks for a db module for plugins and uses that
to verify the schema and to cast rows into the dataclass. If a module isn't
structured as CSR, this feature might not function.

If a plugin isn't CSR, it should hopefully still try to look at __init__.py for
defined dataclass and like for schema verificaiton and row casting.
...
`experience` plugin imports service→service, not cog→cog. Prerequisite for the
plugin-dependency-policy item to be coherent.
...
> custom\_id could never match the button baked into existing messages).
...
is the boundary discipline at the service layer.
 ...
 -  **Boundary rule:** service functions take `db`/`settings` + plain values +
    injected `now`; **no stateful discord objects** (`Member`, `User`,
    `Message`, `Guild`, `Channel`, `Role`, `Asset`, `Interaction`, `Bot`) past
    the controller edge. Pure-data value types (`discord.Embed`, `Permissions`,
 ...
 -  **Extraction order, each step under characterization tests first:**
    1)  `experience._award_exp` (`plugins/experience/cog.py`) — the biggest win;
        returns a plain result, presentation moves to the controller (or later
        an event-bus subscriber — see the core event bus item).

        > **Done** — `plugins/experience/logic.py`:
        > `award_exp(db, *, uid, now)`; cog is a thin controller (characterized
        > by `test_on_message.py`).
 ...
 -  **Enforcement:** an AST-level pytest that walks the plugin tree and asserts
    service modules (`logic.py`/`factory.py`) never `import discord` — fails
    with a readable message naming the file — plus the boundary rule documented
    in `docs/PLUGINS.md`. Enforcement lands after the `experience` pilot proves
    the pattern.
...
`plugins/levels/`). See docs/ARCHITECTURE.md and docs/PLUGINS.md. The poll-view
re-registration item was parked during the reaction→button conversion (2026) to
keep that change focused.
...
Now we have a history of people who have pressed the button. From this, we can
simplify the calls. The recent bakas can be a database call, grouped by user,
summed. From this, we can get the most recent bakas.
...
(characterization test first, per the CSR test-then-extract loop), or restrict
the command help text to single-token durations.
...
> values: id/display\_name/mention/avatar\_url), built at the controller edge
...
Have the LSP do as much hinting as we can for data so development is as
seamless and frictionless as possible. No trying to guess what an object has,
the LSP should be able to trivially determine what it is.

This is especially important for information retrieved from the database. It
should cast into some type that can be trivially understood.
...
All we have is the smoke.py and functest.py. They are unmanagable as they are
monoliths. We need someway to have unit tests more isolated and per-feature
...
Design:
 ...
 -  **Pipeline runner** — executes steps sequentially against the real bot
    ...
    attach wait).
 -  **Definitions** — JSON first:
    `{steps: [{command, args}, ...], channel, schedule}` (validated, testable);
    a shell-like `&&`/`|` DSL parser is a possible later nicety that compiles
    to the same JSON.
...
> guild; “sandbox” means only the `-s` plugin-allowlist mode.
> `scripts/ probe_channels.py` follows the new flags; the legacy PG migration
...
`plugin enable` / `plugin disable` / `plugin list` — flip the flag and
...
oracle, display name/icon, **item-owned consume**). Notes:
 ...
 -  `/inventory` is now a **group** — `/inventory view` (emoji grid via
    `bot.items`; unresolved ids hidden) and
    `/inventory consume <slot> [amount]` (generic, item-owned consume) — and
    **replaces `/frog consume`** (retired). The old namespace **renderer
    registry** (`ItemView`, `register_renderer`/`renderer_for`,
    `frog_renderer`) was removed as redundant. `FrogConsumedEvent` still fires,
    from the frog item's consume handler (so the generic path stays
    frog-agnostic and the future badge system's observation path stays alive).
