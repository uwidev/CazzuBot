...
> **Phase 5 — the five FROG.md species (2026-08-28) is implemented**: their
> consume composition (reaction + role seams), the reaction listener, the
> cluster spawn explosion, and the quarterly “use it or lose it” reset are
> live (see `docs/aegis/plans/2026-08-28-frog-species.md`).
...
### Phase 2 — Species catalog + inventory + effects (`plugins/frogs/`) ✅ implemented 2026-08-14
 ...
 -  Capture flow: roll the species at spawn time (the visible frog **is** the
    ...
    custom\_id (`frog:catch:{cid}:{species_key}`, boot sweep matches the
...
dry-run reports exact counts. Covered by
`tests/core/test_migrate_frog_species.py` (legacy-shape build → plan/migrate →
new shape asserted, idempotence, and a full boot through the real schema guard).
...
### Phase 5 — FROG.md species ✅ implemented 2026-08-28

 -  The five `docs/FROG.md` frogs: Basic (1000), Pog (200), Froggers (50),
    Classy (200), Cluster (300) — weights and rarities resolved from the
    spec (the “Still open” note below no longer applies).
 -  **Consume is item-owned** (owner 2026-08-28): the item composes exp
    (its `frog_exp` oracle) plus its state modifiers; species carry no
    consume declaration. Pog/Froggers publish the ONE reaction effect
    (identity = `FrogEffect.REACTION`, strongest chance wins, additive
    window); Classy publishes the external `CLASSY_ROLE` seam
    (`RoleConverger` registered at load, instant revert on
    `EffectsClearedEvent`).
 -  `plugins/frogs/reactions.py` — a guild-scoped message listener rolling
    the reaction chance (10s in-memory cooldown, no-op until the froggers
    emoji is published).
 -  Cluster is uncatchable: `Species.spawn_effect` short-circuits the spawn
    into a burst of 4–10 Basic frogs across the text channels ±2 around
    the spawn channel (`ClusterEffect`, `spawn_impl` injected at load —
    the edge `effects → factory` would cycle through species).
 -  The quarterly tag now runs a **season reset** — every frog becomes a
    Basic Frog (“use it or lose it”) — not the per-species freeze.
 -  Plan: `docs/aegis/plans/2026-08-28-frog-species.md`.
...
Still open (non-blocking, code-adjustable later): spawn weights + rarity are
now resolved by FROG.md — Basic common/1000, Pog uncommon/200, Froggers
rare/50, Classy rare/200, Cluster special/300 (see the Phase 5 section);
`classy_frog`'s consume behavior beyond exp is the role grant described
there.
