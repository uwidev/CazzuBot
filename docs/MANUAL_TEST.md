...
- [ ] C5. `level set/demo/raw` with a broken JSON template → jsonschema error
  ...
  File "/mnt/hdd/proj/high/in/CazzuBot/.venv/lib/python3.14/site-packages/linkd/solver.py", line 440, in __call
...
- [ ] M3. `channels export/diff/apply/restore` with `--scope-below
      <Category>`: reorder within a category, **slowmode seconds** (timedelta
