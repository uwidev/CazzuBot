...
Every item has:
 ...
 -  a **`description`** — required prose shown on the item's info card
    (`/inventory info`), and
 -  an optional **`consume`** behavior, and
 -  optional **`fields`** — ordered `(label, text)` pairs rendered as labeled
    embed fields on the info card (e.g. `("On consumption", "Grants 10 exp.")`).
...
class BadgeItems(Enum):
    GOLD_COIN = Item(
        ...
        icon="🪙",
        description="A shiny coin minted for Club Cirno members.",
...
render an emoji tag, not a URL image. Once published, the same asset becomes
the info card's **thumbnail** (`/inventory info <slot>` — the bot derives
the emoji's CDN URL from the published reference).
...
class BadgeItems(Enum):
    GOLD_COIN = Item(
        ...
        fields=(("On consumption", "Grants **10** exp."),),
    )
~~~~
