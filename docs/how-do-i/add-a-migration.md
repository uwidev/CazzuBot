...
default changed, re-keying stored values). Purely additive
`CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS` changes don't —
restarting the bot applies those (`docs/add-a-schema.md`).
...
-----------------------------------------
 ...
 -  **Verify inside the transaction when possible** — a parity check before
    commit rolls the whole migration back on mismatch (counter\_events does
    this).
