...
class Leaderboards(customs.cog.Cog):
    def __init__(self, bot):
        super().__init__(bot)

    @commands.command(aliases=['top'])
    async def leaderboards(self, ctx, mode:mode_sort = 'exp', page=1):
        '''
        Shows all members sorted by their experience points.

        Simply providing a number will let you view a page sorted by experience. If you want sort by frogs, be sure to provided the **mode** with **`frogs`**, 
